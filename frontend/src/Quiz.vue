...
function setAnswers(questions) {
  /*
  Установка вариантов ответа для вопросов, нужно для отправки на сервер
  @questions: Вопросы
  */
...
function checkOptionIn(option) {
  /*
  Проверка на наличие варианта ответа в массиве ответов, нужно для переключения чекбоксов
  @option: Вариант ответа
  */
...
function finish(next_step = true) {
  /*
  Завершение квиза и отправка ответа с получением результатов
  @next_step: Переход на следующий вопрос (true or false)
  */
  if (next_step) {
    current_question.value++
    router.replace({query: {question: current_question.value}})
...
function checkAnswer(index) {
  /*
  Проверка на то выбран ли какой-то вариант ответа
...
onMounted(() => {
  // Загрузка квиза при загрузке страницы
...
<template>
  ...
  <main v-if="data.quiz_id" class="d-flex" style="height: 100vh">
    <!-- Окно подтверждения возврата на главную страницу -->
    ...
    <section class="container m-auto">
      ...
      <div v-if="current_question === -1" :class="{'blur': data.theme === 'modern'}"
        ...
        <div class="row g-0">
          ...
          <div class="col">
            <h2 class="card-header p-4 text-center">{{ data.title }}</h2>
        ...
        <div class="card-footer">
          <div class="hstack py-2 gap-2">
            ...
            <button @click="nextQuestion(); startTimer()"
                    ...
                    :class="{'btn-outline-light': data.theme === 'modern', 'btn-outline-dark': data.theme === 'classic'}"
      ...
      <section v-for="(q, index) in data.questions" :key="index">
        <div v-if="current_question === index" :class="{'blur': data.theme === 'modern'}" class="card rounded-4">
          <h2 class="card-header p-4 text-center">{{ q.settings.title }}</h2>
          <div class="card-header p-4 description" v-html="q.settings.text"></div>
          <div class="card-body p-4">
            <!-- Один вариант ответа -->
            <div v-if="q.settings.mode === 0" style="max-height: 20rem" class="overflow-y-scroll p-2 d-flex flex-column gap-2">
            ...
            <div v-if="q.settings.mode === 1" style="max-height: 20rem" class="overflow-y-scroll p-2 d-flex flex-column gap-2">
              <div v-for="(a, i) in q.answers.options" :key="i">
          ...
          <!-- Нижняя панель вопросов с переключением вопросов -->
          <div class="card-footer">
            <div class="hstack py-2 gap-2">
              <span>{{ current_question + 1 }}/{{ data.questions.length }}</span>
              <button
                  ...
                  @click="finish"
                  :class="{'btn-light': data.theme === 'modern', 'btn-dark': data.theme === 'classic'}"
      ...
      <div v-if="current_question === data.questions.length" :class="{'blur': data.theme === 'modern'}"
        ...
        <div class="card-body p-5">
          <h2 v-if="data.requiredName" class="card-title text-center">Спасибо за прохождение,
            {{ answers.user_name }}!</h2>
          <h2 v-else class="card-title text-center">Спасибо за прохождение!</h2>
          <div v-if="data.showResults">
        ...
        <div class="card-footer">
          <div class="hstack py-2 gap-2" @click="router.push({name: 'Home'})">
            <button :class="{'btn-light': data.theme === 'modern', 'btn-dark': data.theme === 'classic'}"
